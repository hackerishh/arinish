const data = {
    general: {
        ...
        SUDO_PERMISSION:
            "```Hey there, I am 💠Arin-ish💠. I guess you were trying to use my commands in``` *{BotsApp.groupName}*```. However, the bot is currently in {worktype} mode. This enables only the owner and sudo users to use the command``` *{commandName}* ```.\n\nIf you are keen to use my features, you can deploy Arin-ish on your own account without much effort, in less than 5 minutes! Check out the links given below.```\n\n🔗instagram.com/djarin.in\n🔗https://github.com/hackerishh/arin",
        ERROR_TEMPLATE:
    ...
    getdp: {
        ...
        EXTENDED_DESCRIPTION: "```Get the profile picture of the group in a group conversation or the profile picture of Arin-ish itself in personal chat. Tag a person in a group to get their profile picture.```",
        IMAGE_CAPTION: "```Here is the display image. Procured by Hacker-ishh.```",
    ...
    help: {
        ...
        HEAD: "🌀 *Hackerishh Menu* 🌀\n```Use .help command for detailed info on a module.```",
        TEMPLATE: "\n\n🤖 *Command* - ```{}```\n💡 *Info* - ```{}```",
        COMMAND_INTERFACE: "🌀 *Arin-ish Command Interface* 🌀\n\n",
